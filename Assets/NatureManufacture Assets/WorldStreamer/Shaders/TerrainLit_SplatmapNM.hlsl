...
float3 SampleNormalGrad(TEXTURE2D_PARAM( textureName, samplerName),
float2 uv, float2 dxuv, float2 dyuv, float scale)
{
...
#undef SampleResults
    ...
    for (int i = 0; i < _LAYER_COUNT; ++i)
    {
        ...
        surfaceData.normalData += normal[i].rgb * weights[i]; // no need to normalize
         outNormal += normal[i].rgb;
        outMasks += masks[i].xyw * weights[i];
    }
   // surfaceData.albedo = 0;//
    //surfaceData.albedo = surfaceData.normalData;
   // surfaceData.normalData = surfaceData.normalData;
