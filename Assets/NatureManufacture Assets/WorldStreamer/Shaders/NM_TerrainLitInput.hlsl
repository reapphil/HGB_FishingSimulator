...
inline void InitializeStandardLitSurfaceData(float2 uv, out SurfaceData outSurfaceData)
{
    ...
    half4 specGloss = SampleMetallicSpecGloss(uv, albedoSmoothness.a);
    outSurfaceData.albedo = half3(0.0h, 0.0h, 0.0h);//    SampleNormal(uv, TEXTURE2D_ARGS(_BumpMap, sampler_BumpMap));

    outSurfaceData.metallic = 0;//    specGloss.r;
    outSurfaceData.specular = half3(0.0h, 0.0h, 0.0h);

    outSurfaceData.smoothness = 0;//    specGloss.a;
    outSurfaceData.normalTS = half3(0.0h, 0.0h, 0.0h);//    SampleNormal(uv, TEXTURE2D_ARGS(_BumpMap, sampler_BumpMap));
    outSurfaceData.occlusion = 1;
    outSurfaceData.emission = 0;
}
