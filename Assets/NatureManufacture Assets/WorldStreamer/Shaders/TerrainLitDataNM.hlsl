...
void GetSurfaceAndBuiltinData(inout FragInputs input, float3 V, inout PositionInputs posInput, out SurfaceData surfaceData, out BuiltinData builtinData RAY_TRACING_OPTIONAL_PARAMETERS)
{
    ...
    if (_metallicMap != -1)
    {
        input.tangentToWorld[0] = float3(1, 0, 0);
        input.tangentToWorld[1] = float3(0, 0, 1);
        input.tangentToWorld[2] = float3(0, 1, 0);
   ...
   // surfaceData.baseColor = 0; //    terrainLitSurfaceData.albedo;
    surfaceData.perceptualSmoothness = 0; // // terrainLitSurfaceData.smoothness;
    surfaceData.metallic = 0; // terrainLitSurfaceData.metallic;
    surfaceData.ambientOcclusion = 0; // terrainLitSurfaceData.ao;
    ...
    surfaceData.materialFeatures = DEBUGVIEW_LIT_SURFACEDATA_NORMAL;
    ...
    {
        surfaceData.baseColor = terrainLitSurfaceData.normalData.xyz;
    ...
    if (_metallicMap != -1)
    {
        ...
        float g = surfaceData.baseColor.g;
        surfaceData.baseColor.g = surfaceData.baseColor.b;
        surfaceData.baseColor.b = g;
    
        surfaceData.baseColor = pow(surfaceData.baseColor * 0.5 + 0.5, 2.2);
             
    
        if (_metallicMap == 1)
            surfaceData.baseColor = SRGBToLinear(terrainLitSurfaceData.metallic);
   // surfaceData.baseColor = SRGBToLinear(terrainLitSurfaceData.metallic);
    
        if (_metallicMap == 2)
            surfaceData.baseColor = SRGBToLinear(terrainLitSurfaceData.ao);
    
        if (_metallicMap == 3)
            surfaceData.baseColor = SRGBToLinear(terrainLitSurfaceData.smoothness);
    }else
    {
        //surfaceData.baseColor = SRGBToLinear(terrainLitSurfaceData.albedo);
